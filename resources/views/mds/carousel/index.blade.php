...
window.mds.registerCarousel = (Alpine) => {
    ...
    Alpine.data('mdsCarousel', (config = {}) => ({
        ...
        // Scroll the TRACK, never the slide. scrollIntoView() walks up and
        // scrolls every scrollable ancestor, so an autoplaying carousel drags
        // the whole page down to itself every few seconds — which is exactly
        // what it did on the docs page until this was measured. scrollBy() on
        // the track touches nothing else.
        //
        // The delta is read off the two rectangles rather than computed from
        // scrollLeft, whose sign and origin in RTL differ between engines and
        // historically between versions of the same engine. Both rects are
        // physical, so their difference is a physical pixel offset that
        // scrollBy() takes directly; picking which edge to align is the only
        // thing direction changes.
        go(i, behavior = null) {
            ...
            // Align the slide's inline-start edge with the track's: the right
            // edge in RTL, the left edge in LTR.
            track.scrollBy({
                left: rtl ? e.right - t.right : e.left - t.left,
                top: 0,
