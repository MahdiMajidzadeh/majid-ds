...
window.mds.registerColorPicker = (Alpine) => {
    ...
    Alpine.data('mdsColorPicker', (config) => ({
        ...
        init() {
            ...
            /*
            | The server can change the colour too — a palette reset, a theme
            | loaded for editing — and that arrives as a Livewire morph, which
            | rewrites the hidden input's `value` ATTRIBUTE. Alpine hears
            | nothing, so the swatch, the sliders and the area cursor all went
            | on showing the old colour while the bound property held another.
            |
            | The observer fires on our own writes too — for
            | input[type=hidden] the value property reflects straight back to
            | the attribute — so what stops a loop is adopt() returning early
            | once the colour already matches, not the write going unseen.
            */
        ...
        // reader: the swatch follows, no input event goes back.
        adopt(raw) {
            const next = (raw ?? '').trim()
            ...
            if (next === '') {
                ...
                this.setFromString(next)
            }

            if (this.$refs.input) this.$refs.input.value = this.empty ? '' : this.output
        },
