...
window.mds.registerRatingInput = (Alpine) => {
    ...
    Alpine.data('mdsRatingInput', (config = {}) => ({
        ...
        /*
        | A morph rewrites the hidden input's `value` attribute when the server
        | changes the bound property — a rating cleared after a failed submit,
        | or one loaded for editing — and nothing about that reaches Alpine, so
        | the stars stayed on the old score. The observer fires on our own
        | writes too — for input[type=hidden] the value property reflects back
        | to the attribute — so the early return in adopt() is what stops a
        | loop, not the write being invisible.
        */
        init() {
        ...
        // Silent: a morph is the server talking, not the reader, so it moves
        // the stars without dispatching an event back. 0 is allowed here
        // though commit() clamps to 1 — the server may legitimately clear it.
        adopt(raw) {
            const n = parseInt(raw ?? '', 10)

            if (Number.isNaN(n)) return

            const clamped = Math.min(Math.max(n, 0), this.max)
